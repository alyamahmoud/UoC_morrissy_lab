...
# heatmap
pdf ('~/Documents/GRNs/pedGBM/plots/Genes_heatmap.pdf', width = 4, height = 4)
...
# pca
pdf('~/Documents/GRNs/pedGBM/plots/Genes_pcas.pdf', width = 4, height = 3)
...
autoplot(prcomp(dat_pca_genes[, 2:4080]), data = dat_pca_genes, colour='clinical_status')
autoplot(prcomp(dat_pca_genes[, 2:4080]), data = dat_pca_genes, colour='patient')
dev.off()
```

the top variable 10% genes were enriched in developmental processes, nervous system development. 
...
rv <- rowVars(as.matrix(dat_proteins)); rv = na.omit(rv)
...
dat_pca_proteins[is.na(dat_pca_proteins)] <- 0
...
```{r}
# exons/DEXSeq
dxd <- readRDS ('~/Documents/GRNs/pedGBM/dxd_pedGBM_DEXSeqAlt.Rds')
dxd <- readRDS ('~/Documents/GRNs/pedGBM/dxdtestForDEUAlt.Rds')
dxr1 <- readRDS ('~/Documents/GRNs/pedGBM/dxr1_pedGBM_DEXSeqAlt.Rds')
dxr1_tab <- readRDS('~/Documents/GRNs/pedGBM/dxr1_tab_pedGBM_DEXSeqAlt.Rds')
...
auc_pedGBM = readRDS('~/Documents/GRNs/pedGBM/int_unsupervised/3.4_regulonAUC.Rds')
regulons_pedGBM = readRDS('~/Documents/GRNs/pedGBM/int_unsupervised/2.6_regulons_asGeneSet.Rds')
```
...
pdf ('~/Documents/GRNs/pedGBM/plots/unsupervised_regulons_heatmap.pdf', width = 4, height = 4)
...
# pca
pdf('~/Documents/GRNs/pedGBM/plots/unsupervised_regulons_pcas.pdf', width = 4, height = 3)
...
autoplot(prcomp(dat_pca_regulons[, 2:463]), data = dat_pca_regulons, colour='clinical_status')
autoplot(prcomp(dat_pca_regulons[, 2:463]), data = dat_pca_regulons, colour='patient')
dev.off()
```
...
# regulons
regulons = unique(names(regulons_pedGBM))
...
```{r}
regulons_private <- regulons [!regulons %in% genes]
regulons_private <- regulons_private [!regulons_private %in% proteins]
regulons_private <- regulons_private [!regulons_private %in% exons]
```
